...
resource "azurerm_virtual_machine_extension" "fetch" {
  ...
  settings = <<SETTINGS
    {
        "fileUris": [
        "https://raw.githubusercontent.com/dcasati/jumpboxes/master/freebsd/yubikey_lockdown.sh"
        ],
        "commandToExecute": "sh yubikey_lockdown.sh -u azureuser -c ${var.client_id} -s ${var.secretkey} -t ${var.token_id} -P"
    }
SETTINGS
}
