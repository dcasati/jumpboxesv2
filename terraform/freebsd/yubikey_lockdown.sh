...
pkg install -y pam_yubico pam_per_user
...
echo "creating the /usr/local/etc/pam.d/sshd file"
cat > /usr/local/etc/pam.d/sshd << EOF
# Route all ssh users through the pam_per_user system
auth required /usr/local/lib/security/pam_per_user.so.1
EOF

echo "creating the /usr/local/etc/pam.d/sshd-yubikey file"

PAM_MOD=requisite
if [ ${NO_PASSWORD} ]; then
	PAM_MOD=sufficient
fi
cat > /usr/local/etc/pam.d/sshd-yubikey << EOF
# auth
# need for a password. The result is Yubikey and Public Key only.
auth        ${PAM_MOD}   /usr/local/lib/security/pam_yubico.so id=${CLIENTID} key=${SECRETKEY} authfile=/usr/local/etc/yubikey_mappings
